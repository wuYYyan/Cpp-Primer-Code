...
void biggies(vector<string> &words, vector<string>::size_type sz)
{
    ...
    //auto wc = partition(words.begin(), words.end(), [sz](const string &s) {return s.size() >= sz;}); //找到第一个长度大于等于目标长度的元素
    //auto count = wc - words.begin(); //使用迭代器的减法进行计数
    auto wc = partition(words.begin(), words.end(), bind(check_size, _1, sz)); //找到第一个长度大于等于目标长度的元素
    auto count = wc - words.begin(); //使用迭代器的减法进行计数
    cout << count << " " << (count > 1 ? "words" : "word") << " of length " << sz << " or longer" << endl;
    for_each(words.begin(), wc, [](const string &s) {cout << s << " ";}); //为了使用lambda表达式，必须使用for_each而不是使用范围for语句
    cout << endl;
}
